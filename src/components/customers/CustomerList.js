...
import { getCustomerDepositHistory, getCustomerPurchaseHistory, getCustomerTransactionsHistory } from "src/statesManagement/store/actions/customer-action";
...
const CustomerList = ({ customers, loading }) => {
  ...
  getCustomerTransactionsHistory({dispatch:dispatch, enqueueSnackbar:enqueueSnackbar, customerId:id, Router:Router})
  };

  const viewDepositHistory = (tableMeta) => (e) => {
  console.log("hello");  
  const id = tableMeta.rowData[5]
  getCustomerDepositHistory({dispatch:dispatch, enqueueSnackbar:enqueueSnackbar, customerId:id, Router:Router})
  };

  const viewPurchaseHistory = (tableMeta) => (e) => {
  console.log("hello");  
  const id = tableMeta.rowData[5]
  getCustomerPurchaseHistory({dispatch:dispatch, enqueueSnackbar:enqueueSnackbar, customerId:id, Router:Router})
  };
  const columns = [
        ...
        customBodyRender: (value, tableMeta, updateValue) => {
          return (
            ...
            loading?( <Button disabled  style={style} variant="contained" component={"h3"} color="primary" >
                  Transaction History
                </Button>):( <Typography  style={style} variant="contained" component={"h3"} color="primary" onClick={viewTransactionHistory(tableMeta)}>
                  Transaction History
                </Typography>)     
